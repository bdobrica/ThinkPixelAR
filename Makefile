...
.PHONY: help deps db-up db-down migrate test-db-migrations test-db-transactions generate fmt fmt-check vet lint static test test-unit test-race vulnerability license hygiene hygiene-test versions-check build image image-smoke agentd-image agentd-image-smoke openapi-check verify baseline-verify
