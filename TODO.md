...
- [ ] AGD-020 Compose the initial runnable AR↔agentd path: concrete bounded authority/materialization expectations, command-specific frame policy and binary wiring; persist bootstrap plan/cleanup intent before publication and UID before projection, then clean up on acceptance/failure/expiry (ADRs 0027–0029). Require an authenticated controlled-process exchange before completion; no permissive default policy. — in progress: durable publication/UID/cleanup coordinator implemented under ADR-0038 ([evidence](docs/evidence/agd-020-bootstrap-delivery.md)); the bounded dispatcher now passes a real mTLS structured-process exchange under ADR-0039 ([evidence](docs/evidence/agd-020-process-dispatch.md)); local admission/frame policies and durable dispatch outcomes are implemented under ADR-0040 ([evidence](docs/evidence/agd-020-admission-replay.md)); bootstrap materialization, atomic acceptance cleanup and tenant worker are implemented under ADR-0041 ([evidence](docs/evidence/agd-020-bootstrap-lifecycle.md)); binary hosting, rotation/heartbeat handling, cleanup hosting and the homelab evidence reader are implemented under ADR-0042 ([evidence](docs/evidence/agd-020-binary-hosting.md)); failure fencing, exact cleanup and bounded authenticated final stop reporting are connected under ADR-0043 ([evidence](docs/evidence/agd-020-failure-handling.md)); ambiguous outcomes retain pending recovery work. Trusted host evidence publication, integrated live acceptance and durable safe-replacement admission remain open.
  [Integrated application acceptance](docs/evidence/agd-020-integrated-acceptance.md) passed 2026-09-23 with the actual agentd image, controlled child, concrete policies and PostgreSQL, including bootstrap/rotation, handshake/capture/status, interrupt, reconnect, duplicate/stale rejection and cleanup. Fixed reconnect STATUS reattaching abandoned capture. Kubernetes readiness/Secret APIs remain fixtures; live acceptance and safe replacement remain open.
- [ ] AGD-019 Commit Phase 4 with protocol/security evidence, including AGD-020 composition and AGD-013 rotation/reconnect. — [evidence review](docs/phase-4-evidence.md) updated 2026-09-23; integrated application acceptance passes, closure awaits AGD-020's live provider acceptance and safe-replacement admission.
