...
- [x] ENG-003 Add dependency/source/license policy and document allowed dependency classes. — completed 2026-08-31, commit `e95946d`, evidence: policy section/reference validation; no committed Go module replacements; pinned Go `go list -deps ./...`; `go test ./...`; `go vet ./...`; `git diff --check`
- [x] ENG-004 Implement typed configuration loading with environment/file support, validation, safe defaults, and secret redaction. — completed 2026-08-31, commit `04c2784`, evidence: pinned Go `go test ./...`; `go test -race ./...`; `go vet ./...`; `go list ./...`; `git diff --check`
- [x] ENG-005 Implement structured logging with canonical request/trace/Session/Execution/Attempt correlation fields and recursive secret redaction. — completed 2026-08-31, commit `805b761`, evidence: pinned Go `go test ./internal/telemetry`; `go test -race ./internal/telemetry`; `go vet ./...`; `go test ./...`; `go test -race ./...`; `go list ./...`; `git diff --check -- internal/telemetry`
- [x] ENG-006 Implement Prometheus metric registry and OpenTelemetry trace initialization with no sensitive payload recording by default. — completed 2026-08-31, commit `8879a2a`, evidence: pinned Go `go test ./internal/telemetry`; `go test -race ./internal/telemetry`; `go vet ./...`; `go test ./...`; `go test -race ./...`; `go list ./...`; Prometheus/OpenTelemetry dependency review; secret-canary/cardinality tests; `git diff --cached --check`
- [x] ENG-007 Implement shared primitives: UUIDv7 IDs, injectable UTC clock, typed errors, authenticated cursors, bounded strings/payloads, and safe enum parsing. — completed 2026-08-31, commit `f5ba14d`, evidence: pinned Go `go test ./internal/primitives ./internal/ports/clock`; `go vet ./internal/primitives ./internal/ports/clock`; `go test -race ./internal/primitives ./internal/ports/clock`; `go test ./...`; `go vet ./...`; `go test -race ./...`; `go list ./...`; cursor tamper/expiry/context-substitution and error secret-canary tests; `git diff --cached --check`
- [x] ENG-008 Implement baseline HTTP server with request IDs, W3C trace extraction, panic recovery, RFC 7807 errors, limits, timeouts, graceful shutdown, `/livez`, `/readyz`, and `/metrics`. — completed 2026-08-31, commit `0672d90`, evidence: pinned Go `go test ./internal/adapters/http ./cmd/thinkpixelar`; `go vet ./internal/adapters/http ./cmd/thinkpixelar`; `go test -race ./internal/adapters/http ./cmd/thinkpixelar`; `go test ./...`; `go vet ./...`; `go test -race ./...`; request-ID validation, W3C valid/invalid parent, panic secret-canary, limit, readiness-disclosure, timeout, and graceful-shutdown tests; `git diff --cached --check`
- [x] ENG-009 Add OpenAPI generation/validation workflow and generated-artifact drift checks. — completed 2026-08-31, commit `b7e2f20`, evidence: pinned Node/Redocly `npm ci`; `npm run openapi:generate`; `npm run openapi:check`; intentional stale-artifact rejection; `npm audit --omit=optional` (0 vulnerabilities); `go test ./...`; `go vet ./...`; `go test -race ./...`; embedded OpenAPI 3.1 test; explicit operation response-matrix validation; `git diff --cached --check`
- [x] ENG-010 Create root Makefile as the stable local/CI command surface. — completed 2026-08-31, commit `d37bbb4`, evidence: `make help`; `make -n verify`; repository-local-cache `make verify`; pinned OpenAPI lint/drift check; `git diff --cached --check`
- [x] ENG-011 Add deterministic unit/race/static/vulnerability/license/build verification targets. — completed 2026-08-31, commit `b31dce1`, evidence: pinned Go `make verify` with repository-local cache; Staticcheck `v0.8.1`; govulncheck `v1.7.0` (no vulnerabilities found); exact Go/npm dependency inventory and license-policy checks; race tests; reproducible-path binary build; OpenAPI lint/drift check; `git diff --cached --check`
- [x] ENG-012 Add PostgreSQL development dependency and explicit migration command skeleton. — completed 2026-08-31, commit `ab26549`, evidence: PostgreSQL `18.6` pinned Docker Official Image tag/index digest review; Docker Compose configuration validation; healthy real-service smoke test and `postgres --version`; explicit fail-closed migration-command tests; pinned Go `go test ./cmd/migrate`; `go vet ./cmd/migrate`; repository-local-cache `make verify`; `git diff --cached --check`
- [x] ENG-013 Create baseline `thinkpixelar` container image using pinned build stages and hardened non-root runtime image. — completed 2026-08-31, commit `5abd47b`, evidence: exact OCI index digest review; real `make image-smoke` build and non-root/read-only/capability-drop/no-new-privileges `/livez` smoke; repository-local-temp `make verify`; `git diff --cached --check`
- [x] ENG-014 Create baseline `thinkpixel-agentd` container/build target separately from vendor agent images. — completed 2026-08-31, commit `e704c6f`, evidence: distinct signal-aware binary and pinned supervisor-only OCI build; real `make agentd-image-smoke` with numeric non-root/read-only/capability-drop/no-new-privileges checks; repository-local-temp `make verify`; `git diff --cached --check`
- [x] ENG-015 Add CI with least-privilege jobs, immutable action pins where practical, dependency cache isolation, and full baseline verification. — completed 2026-09-01, commit `83c1dd1`, evidence: read-only `GITHUB_TOKEN`; non-persisted checkout credentials; official actions pinned to full release SHAs; per-run/per-attempt non-persistent Go/npm caches; `make verify`; real `make image-smoke`; real `make agentd-image-smoke`; static full-SHA/permissions/target checks; `git diff --cached --check`
- [x] ENG-016 Add repository hygiene checks preventing local Workspace state, vendor credentials, kubeconfigs, generated sandbox credentials, binaries, and test secrets from being committed. — completed 2026-09-01, commit `1c3f0bc`, evidence: index-exact `make hygiene`; isolated six-category rejection and clean-example acceptance via `make hygiene-test`; repository-local-cache `make verify`; `git diff --check`
- [x] ENG-017 Start `docs/supported-versions.md` with exact tested component pins obtained from reproducible environment validation. — completed 2026-09-01, commit `7ebcb2d`, evidence: exact Go/Node/npm and immutable Docker/Compose/tool pin validation via `make versions-check`; all `make verify` prerequisites passed with isolated scratch; `git diff --cached --check`
- [x] ENG-018 Verify clean checkout baseline: generation, format, vet/static, unit, race, vulnerability/license, OpenAPI, binary build, and image smoke. — completed 2026-09-01, commit `9ebb64b`, evidence: every `make verify` prerequisite passed across isolated-scratch invocations; real `make image-smoke agentd-image-smoke`; non-mutating CRLF-safe format and OpenAPI generation-drift checks; `git diff --cached --check`
- [x] ENG-019 Commit Phase 1 with evidence under `docs/phase-1-evidence.md`. — completed 2026-09-01, commit `5ab12fa`, evidence: Phase 1 delivery/commit matrix, clean-checkout baseline linkage and explicit deferred-claim boundary in `docs/phase-1-evidence.md`; `make versions-check`; `make fmt-check`; `git diff --cached --check`
...
- [x] DB-003 Add Executions schema/domain including Session binding, authority reference, immutable resolved agent/version evidence, deadline, state, and generation. — completed 2026-09-02, evidence: PostgreSQL 18.6 migration applied twice with ledger/RLS/trigger/FK inspection; exhaustive lifecycle transition tests; focused tests and vet
...
- [x] DB-008 Add Checkpoint metadata schema/domain with immutable integrity and generation references. — completed 2026-09-02, evidence: domain lifecycle binds exact WorkspaceGeneration and immutable signed manifest evidence; PostgreSQL schema enforces tenant/Session lineage, exact generation references, closed credential exclusions, committed integrity immutability, and RLS; focused/full tests and vet
- [x] DB-009 Add Runtime Profile resolution snapshot persistence. — completed 2026-09-02, evidence: immutable per-Execution domain snapshot retains canonical resolved constraints, opaque implementation/version/digest, supported-version tuple, and bounded decision reason; PostgreSQL schema enforces tenant-qualified ownership, RFC 8785 metadata, immutability, and RLS; focused/full tests and vet
- [x] DB-010 Add SandboxBinding and HarnessBinding persistence without leaking Kubernetes/vendor types into the domain. — completed 2026-09-02, evidence: provider-neutral domain bindings preserve exact Attempt fences, opaque external references, immutable runtime/negotiation evidence, stable operation identities, neutral observations, and optimistic versions; PostgreSQL enforces tenant-qualified one-to-one bindings, exact Sandbox/Harness/Attempt lineage, immutable identity, and RLS; focused/full tests and vet
- [x] DB-011 Add append-only ordered Runtime Event persistence with per-stream sequencing and payload/reference bounds. — completed 2026-09-02, evidence: immutable closed-registry domain envelopes enforce UUIDv7 identity, lineage, correlation, retention, recursive JSON and reference bounds; PostgreSQL atomically locks and advances tenant/Session streams, validates exact Execution/Attempt lineage, rejects update/delete, and applies tenant RLS; focused/full tests and vet
- [x] DB-012 Add transaction manager and repository interfaces/implementations. — completed 2026-09-02, evidence: tenant-bound transaction callbacks expose Session, Execution, Attempt, and ordered Runtime Event repositories; optimistic updates classify conflicts, authoritative rows round-trip through validated restore paths, callback failures roll back atomically, and PostgreSQL 18.6 migration-from-empty/integration tests plus focused/full tests, race tests, and vet pass
- [x] DB-013 Add mutation idempotency records with concurrent ownership, request digest validation, response persistence, and expiry. — completed 2026-09-03, evidence: fenced domain ownership/renewal/takeover and terminal replay validation; PostgreSQL full-scope uniqueness, immutable digest/operation identity, RLS, bounded success/failure persistence, terminal-only expiry cleanup, migration-from-empty/reapply, focused/full tests, race tests, and vet
- [x] DB-014 Add transactional outbox with claim lease, retry, replay-safe event identity, and dead-letter metadata. — completed 2026-09-03, evidence: immutable bounded domain envelopes and semantic event identity; fenced claim/takeover, retry, terminal delivery, and sanitized dead-letter transitions; PostgreSQL tenant RLS, available/expired-claim/dead-letter indexes, atomic `SKIP LOCKED` claims, migration checks, repository integration coverage, focused/full tests, race tests, and vet
- [x] DB-015 Add reconciler work-claim/lease mechanism where process coordination cannot rely on AG leases. — completed 2026-09-03, evidence: tenant-scoped durable work identity; bounded scheduling and error classification; atomic `SKIP LOCKED` claims; renewal, retry, completion, expired-lease takeover, and monotonic fencing; stale-worker rejection, RLS, focused/full tests, race tests, and vet
- [x] DB-016 Add cleanup/orphan metadata required for retrying incomplete Sandbox, Workspace, and checkpoint operations. — completed 2026-09-03, evidence: provider-neutral exact cleanup targets for Sandbox, Workspace, snapshot, and checkpoint objects; stable operation/request and ownership-proof digests; orphan visibility, bounded retry/error metadata, optimistic fencing, immutable confirmed/quarantined tombstones, tenant RLS, migration apply/reapply, focused/full tests, race tests, and vet
- [x] DB-017 Add migration-from-empty tests on a real pinned PostgreSQL instance. — completed 2026-09-03, evidence: isolated empty-schema migration and idempotent reapply test validates every embedded migration ledger entry, checksum, tool version, and resulting table on the pinned PostgreSQL 18.6 Compose service; dedicated local/CI target, focused/full tests, race tests, and vet
- [x] DB-018 Add transaction rollback tests including forced failure after external-reference reservation but before final commit. — completed 2026-09-03, evidence: real PostgreSQL tests force callback failure after authority-reference reservation plus RuntimeEvent and outbox writes, prove every write rolls back, prove the unique external reference is immediately reusable, and run on the pinned Compose service in CI
...
- [x] KAS-016 Implement AR reconciler for desired Session/Execution state against actual Sandbox state. — completed 2026-09-20, commit `bfedb75`; [reconciler evidence](docs/evidence/kas-016-reconciler.md); ADR-0016; focused race tests and `make verify`.
- [x] KAS-017 Make sandbox acquisition/release reconciliation restart-safe and idempotent. — completed 2026-09-20, commit `144466d`; [real PostgreSQL evidence](docs/evidence/kas-017-durable-reconciliation.md); ADR-0017; focused database/race tests and `make verify`.
...
- [x] AGD-010 Implement vendor durable-state path registration and checkpoint preparation hooks. — completed 2026-09-22; ADR-0034, [preparation evidence](docs/evidence/agd-010-checkpoint-preparation.md). Bounded process-bound hooks and candidate manifests; trusted filesystem validation/publication remains Phase 6 and authenticated dispatch remains AGD-020.
- [x] AGD-011 Ensure execution-scoped environment/credentials are not copied into persistent vendor-state paths by agentd itself. — completed 2026-09-22; ADR-0035, [credential-exclusion evidence](docs/evidence/agd-011-credential-exclusion.md). Canary regressions verify existing structural no-copy behavior across launch, preparation and restart; future credential injection and trusted checkpoint scanning require their own qualification.
- [x] AGD-012 Implement graceful SIGTERM shutdown and bounded harness termination escalation. — completed 2026-09-22; ADR-0036, [shutdown evidence](docs/evidence/agd-012-shutdown.md). Permanent admission closure, pending-operation cancellation, cooperative close, TERM/KILL and PID-1 orphan reaping; authenticated final delivery and Pod budget validation remain AGD-020.
- [x] AGD-013 Implement authenticated credential rotation and control-plane reconnect after AR process restart, including current-epoch replacement and fenced recovery bootstrap after expiry (ADR-0002). — completed 2026-09-22; ADR-0037, [rotation/reconnect evidence](docs/evidence/agd-013-rotation-reconnect.md). Real mTLS and PostgreSQL coverage; concrete binary/provider delivery wiring remains AGD-020, with fence/replacement when same-Pod recovery delivery is unavailable.
- [x] AGD-014 Add malformed/replayed/oversized message tests. — completed 2026-09-22; [adversarial-message evidence](docs/evidence/agd-014-adversarial-messages.md). Real mTLS raw-wire rejection, replay-policy delivery gates and bounded codec fuzzing; production durable replay reconciliation remains AGD-020.
- [x] AGD-015 Add transport-loss and half-open connection tests. — completed 2026-09-22; [transport-loss evidence](docs/evidence/agd-015-transport-loss.md). Real mTLS socket/server loss and one-way/two-way silent traffic loss verify bounded cancellation and lease cleanup.
- [x] AGD-016 Add sandbox-local privilege tests proving agentd has no unexpected host/Kubernetes capability. — completed 2026-09-22; [privilege evidence](docs/evidence/agd-016-privileges.md). Hardened image probe checks actual PID-1 security state and denied privileged operations; credential exposure fails startup. External Kata qualification remains separate.
...
- [x] AGD-020 Compose the initial runnable AR↔agentd path: concrete bounded authority/materialization expectations, command-specific frame policy and binary wiring; persist bootstrap plan/cleanup intent before publication and UID before projection, then clean up on acceptance/failure/expiry (ADRs 0027–0029). Require an authenticated controlled-process exchange before completion; no permissive default policy. — completed 2026-09-23; [integrated application acceptance and limitations](docs/evidence/agd-020-integrated-acceptance.md), ADRs 0038–0043. Real agentd, child, mTLS, concrete LocalAuthority policies and PostgreSQL cover bootstrap/rotation, handshake/capture/status, interrupt, reconnect, duplicate/stale rejection and cleanup. Live homelab qualification remains a deployment follow-up; safe replacement remains Phase 6 recovery (REC-002–006), with ambiguous outcomes pending.
- [x] AGD-019 Commit Phase 4 with protocol/security evidence, including AGD-020 composition and AGD-013 rotation/reconnect. — completed 2026-09-23; [Phase 4 closure review](docs/phase-4-evidence.md). Initial application scope is complete; live deployment qualification and Phase 6 recovery limitations remain explicitly tracked.
...
- [x] HNS-003 Create HarnessAdapter conformance-test framework independent of Codex. — completed 2026-09-23; [evidence and limits](docs/evidence/hns-003-harness-conformance.md), [suite guide](test/conformance/harness/README.md). Reusable first-turn checks and broken-adapter self-tests; real packaged Codex conformance follows in CDX-015/016.
- [x] HNS-004 Define canonical normalized harness event types and adapter mapping rules. — completed 2026-09-23; [ADR-0046](docs/adr/0046-harness-candidate-events.md), [mapping contract](docs/contracts/harness-events.md), [evidence](docs/evidence/hns-004-harness-events.md). Closed candidate registry, typed payload definitions and conformance declaration checks; concrete Codex decoding/redaction and durable AR publication remain their CDX/Phase 6 work.
- [x] CDX-001 Pin a tested Codex/App Server version and document compatibility/support policy. — completed 2026-09-23; Codex `0.155.0`, [ADR-0047](docs/adr/0047-codex-app-server-version-pin.md), [local protocol evidence](docs/evidence/cdx-001-codex-pin.md). Exact executable/schema fingerprints and isolated real stdio probe; model turns and ARM64/Kata packaging remain subsequent CDX work.
- [x] CDX-002 Build immutable Codex agent runtime OCI image containing required runtime dependencies and `thinkpixel-agentd`. — completed 2026-09-23; [ADR-0048](docs/adr/0048-codex-demo-runtime-image.md), [artifact digests, checks and limits](docs/evidence/cdx-002-codex-image.md). Combined amd64/ARM64 OCI artifact; amd64 protocol/tools/supervisor smoke and ARM64 emulation startup checks. Licensing follows [ADR-0049](docs/adr/0049-use-based-dependency-licensing.md) without a demo-only exception; model turns and native Kata execution remain subsequent CDX work.
...
- [x] CDX-009 Implement turn interrupt/cancellation. — completed 2026-09-26; [interrupt evidence](docs/evidence/cdx-009-interrupt.md). Bounded cooperative pinned turn interruption, interleaved event preservation and replay; authenticated process INTERRUPT retains stop/reap semantics and fallback. Real pinned interrupted turn verified; Kata remains CDX-016.
...
2026-08-31 | `ENG-003` | `e95946d` | policy section/reference validation; no committed Go module replacements; pinned Go `go list -deps ./...`; `go test ./...`; `go vet ./...`; `git diff --check` | Defines allowed dependency classes, trusted-source and immutable-pin rules, default/review/prohibited license treatment, and time-bounded exceptions; automated enforcement remains ENG-011.
2026-08-31 | `ENG-004` | `04c2784` | pinned Go `go test ./...`; `go test -race ./...`; `go vet ./...`; `go list ./...`; `git diff --check` | Adds standard-library-only typed JSON/environment loading, bounded strict validation, loopback-safe development defaults, production fail-closed checks, and explicit secret-safe formatting/redaction.
2026-09-01 | `ENG-017` | `7ebcb2d` | `make versions-check`; all `make verify` prerequisites with isolated scratch; `git diff --cached --check` | Records exact tested repository, development, build, and runtime pins while keeping future Kubernetes/Kata qualification explicitly separate.
2026-09-01 | `ENG-018` | `9ebb64b` | every `make verify` prerequisite across isolated-scratch invocations; real `make image-smoke agentd-image-smoke`; `git diff --cached --check` | Adds CRLF-safe non-mutating format validation and a complete `baseline-verify` source/image aggregate; CI retains split clean-checkout jobs to avoid Docker access in source analysis.
2026-09-01 | `ENG-019` | `5ab12fa` | Phase 1 delivery/commit matrix and evidence-link review; `make versions-check`; `make fmt-check`; `git diff --cached --check` | Closes the engineering-foundation phase without expanding its claim to persistence, cluster, integration, SLO, or release qualification.
